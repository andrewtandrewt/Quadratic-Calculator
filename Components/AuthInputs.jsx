...
export default function AuthInputs() {
  ...
  const calculateHandler = () => {
    ...
    if (
      !Number.isInteger(enteredA) ||
      !Number.isInteger(enteredB) ||
      !Number.isInteger(enteredC)
    ) {
      setOutput("Please enter an integer");
    } else {
      let Radical = enteredB ** 2 - 4 * enteredA * enteredC;
      let xOne = simplify(Radical, 1),
        xTwo = simplify(Radical, 2);
  ...
  const simplify = (num, sign) => {
    ...
    if (radical === 1 && !negRadical) {
      ...
      while (
        Math.abs(factor) <= Math.abs(numerator) ||
        Math.abs(factor) <= Math.abs(denominator)
      ) {
    ...
    //outputs when radical is not simplified
      ...
      while (
        Math.abs(factor) <= Math.abs(denominator) ||
        Math.abs(factor) <= Math.abs(outside) ||
        Math.abs(factor) <= Math.abs(firstTerm)
      ) {
        if (
          firstTerm % factor === 0 &&
